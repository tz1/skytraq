        ...
        for (k = 0; k < nresp; k++) {
            ...
            case 0x80:
                printf
                  ...
                  respbuf[10], respbuf[11], respbuf[12], respbuf[13], respbuf[14], respbuf[17], respbuf[16], respbuf[15]);
            ...
            case 0xae:
                ...
                break;
            case 0xb2:
                printf("AGPS %s %d hours ", respbuf[7] ? "enabled" : "disabled", (respbuf[6] << 8) + respbuf[5]);
                break;
            case 0xb3:
...
static int setagps()
{
    ...
    if (ofd < 0)
        system("wget ftp://skytraq:skytraq@60.250.205.31/ephemeris/Eph.dat -O /tmp/Eph.dat");
    ofd = open("/tmp/Eph.dat", O_RDONLY);
    if (ofd < 0)
        return -2;
